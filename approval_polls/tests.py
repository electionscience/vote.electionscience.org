...
class PollDetailTests(TestCase):
    def test_detail_view_with_a_future_poll(self):
        ...
        response = self.client.get(
            reverse("approval_polls:detail", args=(future_poll.id,))
        )
    ...
    def test_detail_view_with_a_past_poll(self):
        ...
        response = self.client.get(
            reverse("approval_polls:detail", args=(past_poll.id,))
        )
...
class UpdatePollTests(TestCase):
    ...
    def test_poll_details_closed_poll(self):
        ...
        self.client.login(username="user2", password="password123")
        response = self.client.get(
            reverse("approval_polls:detail", args=(poll_closed.id,))
        )
...
class PollEditTests(TestCase):
    ...
    def test_edit_view_with_invalid_poll(self):
        ...
        """
        response = self.client.get(reverse("approval_polls:edit", args=(10000,)))
        self.assertEqual(response.status_code, 404)
    ...
    def test_edit_view_visible_to_other_user(self):
        ...
        self.client.logout()
        self.client.login(username="user2", password="test")
        response = self.client.get(reverse("approval_polls:edit", args=(1,)))
    ...
    def test_new_choices_are_added(self):
        ...
        """
        self.client.post(
            reverse("approval_polls:edit", args=(1,)),
            {
                "choice1": "xxx",
                "linkurl-choice1": "xxx",
                "choice1000": "BBBBB",
...
class SuspendPollTests(TestCase):
    ...
    def test_suspended_tests_cannot_vote(self):
        response = self.client.get(reverse("approval_polls:detail", args=(1,)))
        self.assertContains(
...
class TagCloudTests(TestCase):
    ...
    def test_poll_tags_index(self):
        ...
        response = self.client.get(
            reverse("approval_polls:tagged_polls", args=("New York",))
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
