$(function () {
  ...
  function addChoiceField(numChoiceFields) {
    ...
    input = $("<div class='form-group'><label for='choice" + numChoiceFields + "' class='form-label mt-4'>Option " + numChoiceFields + "</label> <span class='input-group-addon'>\
        <a href='#' class='remove-choice' id='remove-choice" + numChoiceFields + "' title='Remove Choice'>\
          <i class='fa-solid fa-x'></i></a></span>\
      <input class='form-control' type='text' maxlength='100' name='choice" + numChoiceFields + "' id='choice" + numChoiceFields + "' placeholder='...'>\
    </div>");
  ...
  $('.row').on('click', 'a.add-link', function (e) {
      e.preventDefault();
