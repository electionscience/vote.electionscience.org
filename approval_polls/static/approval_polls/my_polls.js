$(function () { 
  ...
  function disableAction(target, action) {
    ...
    "<p>This poll will be " + verbalizeAction(_action) + "." + 
      (action == "Suspend" ? "Suspending the poll will not allow any voting on it." : "") +
      " <button id='confirm-" + _action + "-" + alertDivId + "' type='button' class='btn btn-danger btn-xs'>" + action+"</button>" + 
      " <button id='cancel-" + _action + "-" + alertDivId + "' type='button' class='btn btn-primary btn-xs'>Cancel</button>" + 
    "</p></div>";
